...
class EventController extends Controller
{
    ...
    function EventSummary()
    {
        ...
        $eventMemberDetails = EventRegistrationModel::where('event_id', $eid)->get();
        // ::join('eventinfotable', 'eventregistration.event_id', '=', 'eventinfotable.id')->->select('eventregistration.*', 'eventinfotable.*')
    ...
    function UserRegistrationOnEvent(Request $request)
    {
        ...
        $check_previous_reg = EventRegistrationModel
        ::where('event_id','=', $event_id)
        ->where('user_phone_no', '=', $value)
        ->where('stutus', '=', 'Approved')
        ->orWhere('stutus', '=', 'Pending')
        ->count();

        if($check_host != 1)
        {
            if($check_previous_reg == 1)
            {
                return 4;
            }
            else
            {
                $transaction_check = EventRegistrationModel
    ...
    function userStatus(Request $request)
    {
        ...
        $userStatus = EventRegistrationModel::where('user_phone_no', $phone_no)->where('transaction_no', $transaction_no)->where('id', '=', $reg_id)->get();
    ...
    function updateUserStatus(Request $request)
    {
        ...
        $result = EventRegistrationModel
        ::where('user_phone_no', $user_phone_no)
        ->where('transaction_no', $transaction_no)
        ->where('id', '=', $registration_id)
        ->update(['stutus'=>$userStatus]);
