...
var countdown = JSON.parse(eventData).countdown;

//CountDown part start

const event_reg_last_date = new Date(countdown);

var x = setInterval(function() {

    var now = new Date();
    var distance = event_reg_last_date - now;
    ...
    $('#event_reg_countdown').html(days + "d " + hours + "h " + minutes + "m " + seconds + "s ");
    
    if (distance < 0) {
        clearInterval(x);
        $('#event_reg_countdown').html("EXPIRED");
        confirmPaymentBtnID.disabled = true;
    }
}, 1000);
...
$('#event_pay_acc_no').html(event_pay_acc_no);
...
function RegistrationOnEvent(user_name, user_phone_no, event_name, event_type, event_date, event_id, user_acc_no, transaction_no)
{
    ...
    axios.post(url, {
        ...
        else if(response.status == 200 && response.data == 4)
        {
            flash('You alrady Submited your info. Please wait or contact with Host',{'bgColor' : '#f74134'});
        }
